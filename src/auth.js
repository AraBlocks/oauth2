...
export const {handle, signIn, signOut} = SvelteKitAuth(async (event) => {
	const settings = {
		...
		google: {//uses Google’s OAuth 2.0 via OpenID Connect
			...
			profile(raw) {//we're replacing Auth.js's response parsing function with our own
				return {
	...
	const options = {
		providers: [
			googleProvider(settings.google),
			twitterProvider(settings.twitter),
			githubProvider(settings.github),
			discordProvider(settings.discord),
		],
